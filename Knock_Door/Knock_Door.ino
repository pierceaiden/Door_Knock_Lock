/* Detects patterns of knocks and triggers a motor to unlock
   it, if the pattern is correct.
      ...
      1. Piezo
         Red - A3 (through pullup resistor)
         Black - GND

      2. Program Button
         Leg 1 - Pin 3
         Leg 2 - VCC
         Leg 3 - Resistor to GND
      ...
      7. Red LED
         VCC - Pin 4 (with resistor)
         GND - GND

      8. Green LED
         VCC - Pin 5 (with resistor)
         GND - GND
...
const int allowVCC = 8;           // Gear motor used to turn the lock CW.
const int allowGND = 9;           // Gear motor used to turn the lock CW.
const int allowGNDRev = 10;           // Gear motor used to turn the lock ACW.
const int allowVCCRev = 11;           // Gear motor used to turn the lock ACW.
...
const int unlockTurnTime1 = 9000;      // milliseconds that we run the motor to get it to go a half turn.
...
void setup() {
  pinMode(allowGND, OUTPUT);
  pinMode(allowVCC, OUTPUT);
  pinMode(allowGNDRev, OUTPUT);
  pinMode(allowVCCRev, OUTPUT);
  pinMode(redLED, OUTPUT);
  pinMode(greenLED, OUTPUT);
  pinMode(programSwitch, INPUT);

  Serial.begin(9600);                     // Uncomment the Serial.bla lines for debugging.
  Serial.println("System Ready");       // but feel free to comment them out after it's working right.
  Serial.println("Program KNOCK!");
  
  digitalWrite(greenLED, HIGH);      // Green LED on, everything is go.
  digitalWrite(allowGND, LOW);
  digitalWrite(allowVCC, LOW);
  digitalWrite(allowGNDRev, LOW);
  digitalWrite(allowVCCRev, LOW);
}
...
void listenToSecretKnock(){
  Serial.println("Knock Detected. Listening for pattern...");
  ...
  if (programButtonPressed==false){             // only if we're not in progrmaing mode.
    if (validateKnock() == true && lockStatus == 1){
      triggerDoorUnlock();
    }
    else if (validateKnock() == true && lockStatus == 0){
      triggerDoorLock();
    }
    else {
      Serial.println("Wrong Secret KNOCK");
    ...
    Serial.println("New Lock Pattern stored");
    digitalWrite(redLED, LOW);
    digitalWrite(greenLED, HIGH);
    for (i=0;i<3;i++){
      delay(100);
...
void triggerDoorUnlock(){
  ...
  digitalWrite(allowGND, HIGH);
  digitalWrite(allowVCC, HIGH);
  digitalWrite(greenLED, HIGH);            // And the green LED too.

  delay (unlockTurnTime1);                    // Wait a bit.

  Serial.println("Door Unlocked!");
  digitalWrite(allowGND, LOW);            // Turn the motor off.
  digitalWrite(allowVCC, LOW);
  lockStatus = 0;
...
void triggerDoorLock(){
  ...
  // turn the motor on for a bit.
  digitalWrite(allowVCCRev, HIGH);
  digitalWrite(allowGNDRev, HIGH);
  digitalWrite(greenLED, HIGH);            // And the green LED too.

  delay (lockTurnTime);       // Wait a bit.
  
  Serial.println("Door Locked!");
  digitalWrite(allowGNDRev, LOW);            // Turn the motor off.
  digitalWrite(allowVCCRev, LOW);
  lockStatus = 1;
